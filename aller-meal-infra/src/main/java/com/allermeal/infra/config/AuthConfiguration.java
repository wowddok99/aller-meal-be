...
public class AuthConfiguration {
	...
	AesGcmEmailEncryptor emailCipher(
		@Value("${aller-meal.auth.email-encryption-key}") String encodedKey,
		@Value("${aller-meal.auth.email-encryption-key-version:v1-local}") String keyVersion,
		SecureRandom secureRandom
	) {
	...
	PasswordHasher passwordHasher(
		SecureRandom secureRandom,
		@Value("${aller-meal.auth.password-hash-iterations:210000}") int iterations
	) {
	...
	LoginAttemptStore loginAttemptStore(
		...
		@Value("${aller-meal.auth.login-failure-window:15m}") Duration failureWindow,
		@Value("${aller-meal.auth.login-lock-duration:15m}") Duration lockDuration,
		@Value("${aller-meal.auth.login-max-failures:5}") int maxFailures
	) {
	...
	AccessTokenIssuer accessTokenIssuer(
		@Value("${aller-meal.auth.access-token-signing-key}") String encodedKey,
		Clock clock,
	...
	EmailVerificationMailSender emailVerificationMailSender(
		...
		@Value("${aller-meal.auth.email-from:no-reply@allermeal.local}") String from,
		@Value("${aller-meal.auth.email-verification-base-url}") String verificationBaseUrl
	) {
	...
	PasswordResetMailSender passwordResetMailSender(
		JavaMailSender mailSender,
		@Value("${aller-meal.auth.email-from:no-reply@allermeal.local}") String from,
		@Value("${aller-meal.auth.password-reset-base-url}") String passwordResetBaseUrl
	) {
	...
	EmailVerificationRequester emailVerificationRequester(
		...
		@Value("${aller-meal.auth.email-verification-token-ttl:30m}") Duration tokenTtl
	) {
	...
	LoginService loginService(
		...
		LoginAttemptStore loginAttemptStore,
		@Value("${aller-meal.auth.access-token-ttl:15m}") Duration accessTokenTtl,
	...
	PasswordResetRequester passwordResetRequester(
		...
		@Value("${aller-meal.auth.password-reset-token-ttl:30m}") Duration tokenTtl
	) {
