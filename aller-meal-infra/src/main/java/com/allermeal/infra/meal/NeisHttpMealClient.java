...
public class NeisHttpMealClient implements NeisMealClient {
	...
	public NeisHttpMealClient(
		...
		@Value("${aller-meal.neis.meal.max-payload-bytes:1048576}") int maxPayloadBytes,
		@Value("${aller-meal.neis.meal.max-attempts:3}") int maxAttempts,
		@Value("${aller-meal.neis.meal.initial-backoff:200ms}") Duration initialBackoff,
		@Value("${aller-meal.neis.meal.backoff-multiplier:2.0}") double backoffMultiplier,
		@Value("${aller-meal.neis.meal.circuit-breaker.failure-rate-threshold:50}") float failureRateThreshold,
		@Value("${aller-meal.neis.meal.circuit-breaker.sliding-window-size:10}") int slidingWindowSize,
		@Value("${aller-meal.neis.meal.circuit-breaker.minimum-calls:5}") int minimumCalls,
		@Value("${aller-meal.neis.meal.circuit-breaker.open-duration:30s}") Duration openDuration
	) {
		...
		try {
			...
			if (response.statusCode() < 200 || response.statusCode() >= 300) {
				...
				throw new MealCollectionException(
					httpFailureCode(response.statusCode()), "NEIS 급식 호출에 실패했습니다.");
			}
			try {
	...
	private void saveLog(
		...
		try {
			...
			// 외부 API 로그 저장 실패가 수집 본 흐름을 막지 않는다.
		}
	}
