...
public class MinioRawPayloadStorage implements RawPayloadStorage, RawPayloadObjectRemover {
	...
	public MinioRawPayloadStorage(
		...
		@Value("${aller-meal.minio.raw-bucket:aller-meal-raw}") String bucket
	) {
	...
	public RawObjectMetadata store(RawPayload payload) {
		...
		try {
			...
			throw new RawPayloadStorageException("원본 객체 metadata 저장에 실패했습니다.", exception);
		}
	}
	...
	public void remove(String objectKey) {
		try {
			...
			throw new RawPayloadStorageException("MinIO 원본 payload 삭제에 실패했습니다.", exception);
		}
	}
