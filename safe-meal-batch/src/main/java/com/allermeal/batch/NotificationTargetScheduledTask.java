...
@Component
@ConditionalOnProperty(prefix = "safe-meal.notification-target.scheduler", name = "enabled", havingValue = "true")
public class NotificationTargetScheduledTask {
	...
	public NotificationTargetScheduledTask(
		...
		@Value("${safe-meal.notification-target.scheduler.lookback:PT5M}") Duration lookback
	) {
		this.scheduler = scheduler;
		this.lookback = lookback;
	}

	@Scheduled(fixedDelayString = "${safe-meal.notification-target.scheduler.fixed-delay:PT1M}")
