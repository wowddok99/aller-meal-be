...
@Component
@ConditionalOnProperty(prefix = "safe-meal.collection.scheduler", name = "enabled", havingValue = "true")
public class RegisteredSchoolMealCollectionScheduledTask {
	...
	public RegisteredSchoolMealCollectionScheduledTask(
		...
		@Value("${safe-meal.collection.scheduler.days-ahead:7}") int daysAhead
	) {
		this.scheduler = scheduler;
		this.daysAhead = daysAhead;
	}

	@Scheduled(fixedDelayString = "${safe-meal.collection.scheduler.fixed-delay:PT1H}")
	public void collect() {
