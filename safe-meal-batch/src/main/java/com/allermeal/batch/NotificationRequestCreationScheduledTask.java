...
@Component
@ConditionalOnProperty(prefix = "safe-meal.notification-request.scheduler", name = "enabled", havingValue = "true")
public class NotificationRequestCreationScheduledTask {
	...
	public NotificationRequestCreationScheduledTask(
		...
		@Value("${safe-meal.notification-request.scheduler.limit:100}") int limit
	) {
		this.notificationRequestCreationService = notificationRequestCreationService;
		this.limit = limit;
	}

	@Scheduled(fixedDelayString = "${safe-meal.notification-request.scheduler.fixed-delay:PT1M}")
	public void create() {
