...
@Component
@ConditionalOnProperty(prefix = "aller-meal.collection.run-once", name = "enabled", havingValue = "true")
public class MealCollectionApplicationRunner implements ApplicationRunner {
	...
	public MealCollectionApplicationRunner(
		...
		@Value("${aller-meal.collection.run-once.school-id}") UUID schoolId,
		@Value("${aller-meal.collection.run-once.meal-date}") LocalDate mealDate,
		@Value("${aller-meal.collection.run-once.meal-type}") MealType mealType
	) {
